...
if(NpcHandler == nil) then
	local function getConversationKey(cid)
		if type(cid) == 'userdata' then
			local ok, value = pcall(function() return cid:getId() end)
	...
	function NpcHandler:addFocus(newFocus)
		if(newFocus ~= 0 and not isPlayer(newFocus) and not isCreature(newFocus)) then
			return
		end

		self.focuses = newFocus
		self.focus = newFocus
		self.focusKey = getConversationKey(newFocus)
	...
	function NpcHandler:isFocused(focus, creatureCheck)
		local focusKey = getConversationKey(focus)
		local creatureCheck = creatureCheck or false
		if(creatureCheck or isPlayer(self.focuses) or isCreature(self.focuses)) then
			return self.focusKey == focusKey
		end
