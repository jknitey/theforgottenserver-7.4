...
if(Modules == nil) then
	...
	function StdModule.learnSpell(cid, message, keywords, parameters, node)
		...
		if(isPremium(cid) or not(parameters.premium)) then
			...
			elseif(getPlayerLevel(cid) < parameters.level) then
				...
				local hasVocation = true
				if parameters.vocation ~= nil then
					if type(parameters.vocation) == 'function' then
						hasVocation = parameters.vocation(cid)
					elseif type(parameters.vocation) == 'number' then
						local playerVocation = getPlayerVocation(cid)
						hasVocation = playerVocation == parameters.vocation or playerVocation == getPromotedVocation(parameters.vocation)
					end
				end
	...
	function StdModule.travel(cid, message, keywords, parameters, node)
		...
		elseif((parameters.cost or 0) > 0 and not doPlayerRemoveMoney(cid, parameters.cost)) then
			npcHandler:say('You don\'t have enough money.', cid)
		elseif(parameters.destination == nil) then
			npcHandler:say('I cannot sail there right now.', cid)
		else
	...
	function ShopModule:addBuyableItem(names, itemid, cost, subType, realName)
		...
		if(not v) then
			print('[Warning - ' .. tostring(getCreatureName(getNpcId()) or 'unknown') .. '] NpcSystem:', 'Invalid buyable item id:', itemid, realName or '')
			return false
		end
		if(SHOPMODULE_MODE ~= SHOPMODULE_MODE_TALK) then
			local item = {
	...
	function ShopModule:addBuyableItemContainer(names, container, itemid, cost, subType, realName)
		if(names ~= nil) then
			local v = getItemInfo(itemid)
			if(not v) then
				print('[Warning - ' .. tostring(getCreatureName(getNpcId()) or 'unknown') .. '] NpcSystem:', 'Invalid buyable container item id:', itemid, realName or '')
				return false
			end
			local parameters = {
				container = container,
	...
	function ShopModule:addSellableItem(names, itemid, cost, realName)
		local v = getItemInfo(itemid)
		if(not v) then
			print('[Warning - ' .. tostring(getCreatureName(getNpcId()) or 'unknown') .. '] NpcSystem:', 'Invalid sellable item id:', itemid, realName or '')
			return false
		end
		if(SHOPMODULE_MODE ~= SHOPMODULE_MODE_TALK) then
			local item = {
	...
	function ShopModule:callbackOnSell(cid, itemid, subType, amount, ignoreEquipped, dummy)
		...
		if(not itemInfo) then
			print('[Warning - ' .. getCreatureName(getNpcId()) .. '] NpcSystem:', 'Invalid sell callback item id:', itemid)
			return false
		end

		if(subType < 1 or itemInfo.stackable) then
			subType = -1
		end
