...
if msgcontains(msg, 'promotion') or msgcontains(msg, 'promote') then
	NPCSay("Do you want to be promoted in your vocation for 20000 gold?", 1)
	...
	if getPlayerVocation(cid) >= 1 and getPlayerVocation(cid) <= 4 then
		if getPlayerLevel(cid) >= 20 then
			if isPremium(cid) == TRUE then
				...
				NPCSay("Congratulations! You are now promoted. Visit the sage Eremo for new spells.", 1)
				ishebad_talk_state = 0		
				else
				NPCSay("You do not have enough money.", 1)
				ishebad_talk_state = 0			
				end
			else
			NPCSay("You need a premium account in order to promote.", 1)
			ishebad_talk_state = 0				
			end
		else
		NPCSay("You need to be at least level 20 in order to be promoted.", 1)
		ishebad_talk_state = 0			
		end
	else
	NPCSay("You are already promoted.", 1)
	ishebad_talk_state = 0	
	end

elseif msgcontains(msg, '') then
	NPCSay("I don't understand? are you trying to say \"promote\" or something?", 1)
	ishebad_talk_state = 0	
end	
end
